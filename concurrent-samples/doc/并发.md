...
`lock.newCondition()`的作用其实时new了一个在`AQS`内部声明的`ConditionObject`对象，它是`AQS`的内部类。**在每个条件变量内部维护了一个条件队列（单项链表），用来存放条件变量的`await()`方法时被阻塞的线程。**一个`Lock`可以对应多个条件变量。当`AQS`同步队列中正在执行的线程调用了`condition`的`await()`方法时，就会将该线程加入到条件队列中。

#### 4.3队列

##### `ConcurrentLinkedQueue`

`ConcurrentLinkedQueue`是一个无界队列，底层使用单向链表来保存队列元素，每个元素被包装成一个`Node`节点；队列是靠头、尾节点来维护的，创建队列时头、尾节点指向一个`item`为null的哨兵节点。第一次执行`peek`或者`poll`操作时会把head指向第一个真正的队列元素。

- offer操作

  在队列末尾添加一个元素，通过`cas`来保证线程安全性。如果元素为null，则抛异常。
